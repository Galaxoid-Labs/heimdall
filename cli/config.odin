...
Project :: struct {
	...
	category:     string, // app category (platform-specific value)
	min_macos:    string, // LSMinimumSystemVersion (macOS)
	bundle_icon:  string, // path to an app icon (.icns/.png on macOS)
...
collect_toml_line :: proc(values: ^map[string]string, raw: string, section: string) -> string {
	...
	val := strings.trim(strings.trim_space(line[eq + 1:]), "\"'")
