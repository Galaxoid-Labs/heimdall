...
// macOS `.app` assembly (Info.plist, executable, icon, PkgInfo), optionally signed
// and notarized. Called from cmd_bundle on Darwin.
bundle_macos :: proc(p: Project, do_sign, adhoc, notarize: bool) {
	// Required config.
...
Add a reverse-DNS bundle id to heimdall.toml, e.g.:
  ...
  version    = "1.0.0"`,
	...
	// Icon (optional): .icns copied as-is, .png converted via sips+iconutil.
	has_icon := false
	if p.bundle_icon != "" {
		has_icon = bundle_icon_macos(p.bundle_icon, resources)
	}
	...
	// Optionally sign (and notarize) the freshly-assembled bundle.
	if do_sign {
...
@(private = "file")
bundle_icon_macos :: proc(icon: string, resources: string) -> bool {
	if !file_exists(icon) {
